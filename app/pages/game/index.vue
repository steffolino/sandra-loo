    ...
    <div v-else class="space-y-6">
      ...
      <section v-if="showMilestonePicker" class="card p-5">
        ...
        <div class="mt-4 grid gap-4 sm:grid-cols-3">
          ...
          >
            <img
              :src="milestoneArtwork(option.id)"
              :alt="option.label"
              class="h-44 w-full rounded-2xl object-cover bg-white/70"
            >
            <div class="mt-3 text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-slate-400">
              {{ option.icon }}
            </div>
      ...
      <section v-else class="card p-5">
        ...
        <GameFpsRunScene
          ...
          :points-per-step="config?.pointsPerStep ?? 100"
          :bladder-relief-bonus="activeRewardBonuses.bladderReliefBonus"
          :igitt-shield-bonus="activeRewardBonuses.igittShieldBonus"
          :score-bonus="activeRewardBonuses.scoreBonus"
...
import type { GameConfig, MilestoneOption, Reward } from '../../../shared/types'
import { useGame } from '../../composables/useGame'
import { calculateRewardBonuses } from '../../shared/game'
...
const activeRewardBonuses = computed(() => calculateRewardBonuses(gameState.value.equippedRewards))
...
function milestoneArtwork(id: MilestoneOption['id']) {
  if (id === 'shoe-shop') return '/game/locations/shoe_store_new_alpha_cropped.png'
  if (id === 'grocery-store') return '/game/locations/grocery_market_new_alpha_cropped.png'
  return '/game/locations/pharmacy_new_alpha_cropped.png'
}
</script>
